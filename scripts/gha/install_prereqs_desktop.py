    ...
    if not args.running_only:
        ...
        if not utils.is_command_installed("protoc"):
            if utils.is_linux_os():
                ...
                utils.run_command(
                    ["apt", "install", "-y", "protobuf-compiler"], as_root=True
                )
        ...
        if args.ssl == "openssl":
            if utils.is_linux_os():
                ...
                utils.run_command(
                    ["choco", "install", "openssl", "-r", "--version", "3.1.1"]
                )
        ...
        if not utils.is_command_installed("clang-format"):
            if utils.is_linux_os():
                ...
                utils.run_command(
                    ["apt", "install", "-y", "clang-format"], as_root=True
                )
            elif utils.is_mac_os():
                # brew install protobuf
        ...
        if utils.is_linux_os():
            ...
            if not (
                "gcc version 9." in gcc_ver.stderr
                or "gcc version 10." in gcc_ver.stderr
            ):
                if not utils.is_command_installed(
                    "gcc-10"
                ) or not utils.is_command_installed("g++-10"):
                    utils.run_command(
                        ["apt", "install", "-y", "gcc-10", "g++-10"], as_root=True
                    )
                utils.run_command(
                    [
                        "update-alternatives",
        ...
        if utils.is_linux_os():
            if not utils.glob_exists("/usr/share/doc/python3.*-venv"):
                utils.run_command(
                    ["apt", "install", "-y", "python3-venv"], as_root=True
                )
        ...
        utils.run_command(
            [
                "python3" if utils.is_command_installed("python3") else "python",
                "-m",
                "pip",
                "install",
                "-r",
...
def parse_cmdline_args():
    ...
    parser.add_argument(
        "--arch",
        default=None,
        help="Install support libraries to build a specific architecture (currently supported: x86)",
    )
    parser.add_argument(
        "--running_only",
        action="store_true",
        help="Only install prerequisites for running, not for building",
    )
    parser.add_argument(
        "--gha_build",
        action="store_true",
        default=None,
        help="Set this option when building on GitHub, changing some prerequisite installation behavior",
    )
    parser.add_argument(
        "--ssl",
        default="openssl",
        help="Which SSL is this build using (supported: openssl, boringssl)",
    )

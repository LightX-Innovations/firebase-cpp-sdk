...
def _install_cpp_dependencies_with_vcpkg(arch, msvc_runtime_library, use_openssl=False):
    ...
    vcpkg_response_files_dir_path = os.path.join(
        os.getcwd(), "external", "vcpkg_custom_data", "response_files"
    )
    if use_openssl:
        vcpkg_response_files_dir_path = os.path.join(
            vcpkg_response_files_dir_path, "openssl"
        )

    vcpkg_response_file_path = os.path.join(
        vcpkg_response_files_dir_path, "{0}.txt".format(vcpkg_triplet)
    )
    ...
    utils.run_command(
        [
            vcpkg_executable_file_path,
            "install",
            "@" + vcpkg_response_file_path,
            "--disable-metrics",
        ],
        check=True,
    )
...
def install_cpp_dependencies_with_vcpkg(
    arch, msvc_runtime_library, cleanup=True, use_openssl=False
):
    ...
    if not success:
        print(
            "Installation was not successful but auto fix was attempted. "
...
def main():
    ...
    if not args.disable_vcpkg:
        # Install C++ dependencies using vcpkg
        install_cpp_dependencies_with_vcpkg(
            args.arch,
            args.msvc_runtime_library,
            cleanup=True,
            use_openssl=args.use_openssl,
        )

    if args.vcpkg_step_only:
        print(
            "Exiting without building the Firebase C++ SDK as just vcpkg step was requested."
        )
        return
...
def parse_cmdline_args():
    parser = argparse.ArgumentParser(
        description="Install Prerequisites for building cpp sdk"
    )
    parser.add_argument(
        "-a", "--arch", default="x64", help="Platform architecture (x64, x86, arm64)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--linux_abi", default="legacy", help="C++ ABI for Linux (legacy or c++11)"
    )
    parser.add_argument("--build_dir", default="build", help="Output build directory")
    parser.add_argument(
        "--build_tests", action="store_true", help="Build unit tests too"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Enable verbose CMake builds."
    )
    parser.add_argument(
        "--disable_vcpkg",
        default="True",
        action="store_true",
        help="Disable vcpkg and just use CMake.",
    )
    parser.add_argument(
        "--vcpkg_step_only",
        action="store_true",
        help="Just install cpp packages using vcpkg and exit.",
    )
    parser.add_argument("--config", default="Release", help="Release/Debug config")
    parser.add_argument(
        "--target",
        nargs="+",
        help="A list of CMake build targets (eg: firebase_app firebase_auth)",
    )
    parser.add_argument(
        "--target_format",
        default=None,
        help="(Mac only) whether to output frameworks (default) or libraries.",
    )
    parser.add_argument(
        "--use_openssl",
        action="store_true",
        default=None,
        help="Use openssl for build instead of boringssl",
    )
    parser.add_argument(
        "--firestore_dep_source",
        default="RELEASED",
        help='Where to get Firestore C++ Core source code. "RELEASED"/"TIP"/(Git tag/branch/commit)',
    )
    parser.add_argument(
        "--gha_build",
        action="store_true",
        default=None,
        help="Set to true when building on GitHub, for metric tracking purposes (also changes some prerequisite installation behavior).",
    )
